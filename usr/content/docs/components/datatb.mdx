...
<Example title="Headless Source Components" codeWidth='full'>
  <Fragment slot="intro">
    `CsvSource`, `JsonSource`, `DirectusSource`, and `ApiSource` are also exported from `@core/components/DataTb` as standalone, headless data-loaders — they render nothing and instead call `onDataLoad` / `onLoadingChange` / `onError` props, so you can drive **your own** custom UI (not a table at all) with the same fetching logic `DataTb` uses internally. They are not children of `<DataTb>` — it has no children/composition API.
  </Fragment>
