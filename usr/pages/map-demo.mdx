...
The `Map` component renders an interactive map using MapLibre GL JS. Vector layers can be added via the `vectorLayers` prop or shorthand props like `geojson`, `csv`, `json`, etc. Layers can include searchable fields for advanced filtering.
...
## Vector Layer Search Functionality

<Example title="Example: Vector Layers with Search">
  <Fragment slot="intro">
    This example demonstrates vector layers with searchable fields. Click the search icon next to a layer name in the layer control to open the search interface. The search supports both simple free-text search across all fields and advanced search with field-operator-value triplets.
  </Fragment>
  <Fragment slot="code">
  ```jsx
  <Map 
