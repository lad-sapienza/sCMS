...
The `Map` component renders an interactive map using [MapLibre GL JS](https://maplibre.org/). Overlay layers — GeoJSON, CSV, Directus, a generic API, or raster XYZ/WMS tiles — can be added via the `overlayLayers` prop or shorthand props like `geojson`, `csv`, `directus`, etc. Layers can include searchable fields for advanced filtering.
