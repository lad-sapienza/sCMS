...
<MapComp
  ...
  <VectorLayer 
    ...
    checked={true}
    popupTemplate={
    (attribs) => `<p><strong>${attribs["Item Label"]}</strong> 
    ${attribs["Site Name"]}
    <br>
    Date: ${attribs["Early BCE/CE"]} — ${attribs["Late BCE/CE"]}
    ` }
    pointToLayer={
      (f, ll) => {
        return L.circleMarker(ll, {
          radius: 4,
          fillColor: !Object.hasOwn(f.properties, "Late BCE/CE") ? '#ebebeb' : (f.properties["Late BCE/CE"] < 0 ? "#FE04FC" : '#1B64F7'),
          color: "#000",
          weight: 1,
          opacity: 1,
          fillOpacity: 0.8
  ...
  <div className="col-sm-4">
    ...
    <MapComp height="400px" center="43,13.32,11" baseLayers="OSM,EsriSatellite">
      <VectorLayer
        ...
        dQueryString="filter[provincia][_eq]=Macerata"
        popupTemplate={attribs => {
          return `<p><strong>${attribs.toponimo}</strong>`
        }}
        geoField="coordinates"
